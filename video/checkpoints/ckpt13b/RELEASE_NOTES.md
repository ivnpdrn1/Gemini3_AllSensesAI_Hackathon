...
### Regression Harness Note (2026-02-01)

**Issue**: The original hardcoded ckpt1 hash constant in `run-regression-local.ps1` was incorrect, causing false FAIL reports.

**Root Cause**: The hardcoded constant (`015EC...`) was the hash of the video variant file, not the baseline production file.

**Verified Ckpt1 Hash**: The correct baseline production file hash is:
```
3EE4EC5F221BEE1B15D4B467A35D59D8C567BF88C380341CD5A5D63E27F98B8D
```

**Fix**: Regression script now reads from canonical hash proof files instead of hardcoded constants:
1. Primary: `checkpoints/ckpt1-baseline-hash.txt`
2. Fallback: `regression/known-baseline-hash.txt`

**Non-Destructive Guarantee**: This fix only updates the regression test harness. The baseline production file was never modified and remains unchanged from checkpoint 1.

**Documentation**: See `regression/HISTORY.md` and `regression/ckpt1-hash-proof.md` for complete incident details.
...
**Note**: This is the correct hash of the baseline production file. The original regression script incorrectly used `015EC...` which was the video variant file hash.
